...
<dt>Baked Diffuse Tint and Intensity</dt>
...
<dt>Reflections Tint and Intensity</dt>
...
<dt>Surface Thickness Threshold</dt>
<dd>Screen space reflection uses the distance from the camera to the surfaces facing the camera, but it has no way to tell the thickness of the objects behind those surfaces. When a reflection would be expected to show the underside or back side of a surface, the effect attempts to extrapolate something to show based on the part of the surface that is visible, and based on a "hint" that you give it in these settings.

The **Max** threshold represents how "thick" a surface has to be for it to be considered a reflection intersection during the ray march.

The **Min** value is the distance in screen space that the two depth values are measured from before doing the threshold test against the max value.

<!-- TODO: this is still really unclear! -->
</dd>

<dt>Border Bending Enabled</dt>
<dd>Screen space reflection is usually unable to produce reflections near the edges of the viewport, because the angle of reflection would cause the reflection to show objects that are not in the frame. This parameter determines what to do in these border areas.

Values closer to `0` cause the reflections to remain accurate as far as possible, then to fade out near the border of the viewport.

Values closer to `1` cause the reflections in the area near the edge of the frame to be bent or dragged outward toward the frame, so that the surface continues to reflect objects that are in the frame right up to the edge.

For example, see the left side of the following images. On the left, the reflections fade out approximately where the corner of the ramp is reflected. In the image on the right, the reflection of that corner is pulled farther over toward the left edge of the frame.

![Screen space reflection bending](../../images/shading_environment_ssr_bend.jpg)
</dd>
